...
$image = [
    'full' => wp_get_attachment_image_url(carbon_get_theme_option( 'header_logo' ), $size='logo'),
    'alt' => get_post_meta(carbon_get_theme_option( 'header_logo' ), '_wp_attachment_image_alt', TRUE),
    'eng' => wp_get_attachment_image_url(carbon_get_theme_option( 'eng' ), 'full'),
    'rus' => wp_get_attachment_image_url(carbon_get_theme_option( 'rus' ), 'full'),
    'kg' => wp_get_attachment_image_url(carbon_get_theme_option( 'kg' ), 'full'),
];
?>
...
<header class="page-header" <?php if(is_user_logged_in()) echo 'style="margin-top:32px;"' ?>>
    <div class="container">
        ...
        <?php endif; ?>
        <div class="dropdown" <?php if(  is_front_page()):?>style="display: none"<?php endif;?>>
            <a href="http://www.civildevcenter.org/"><button class="dropbtn" style="background: url(<?= $image['rus'];?>)"></button></a>
        </div>

        <div class="dropdown" <?php if(  is_page( 52 )):?>style="display: none"<?php endif;?>>
                <a href="http://www.civildevcenter.org/kyrgyzcha/"><button class="dropbtn" style="background: url(<?= $image['kg'];?>)"></button></a>
        </div>


        <div class="dropdown" <?php if(  is_page( 75 )):?>style="display: none"<?php endif;?>>
                <a href="http://www.civildevcenter.org/english/"><button class="dropbtn" style="background: url(<?= $image['eng'];?>)"></button></a>
        </div>
